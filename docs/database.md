...
| `currencies` | M2 | 币种主数据 | 未创建 |
| `exchange_rates` | M2/P2 | 汇率（扩展） | 未创建 |
| `customers` | M2 | 客户 | 未创建 |
| `suppliers` | M2 | 供应商 | 未创建 |
| `materials` | M2 | 物料/产品 | 未创建 |
| `warehouses` | M2 | 仓库 | 未创建 |
| `inventory_balances` | M2 | 库存余额 | 未创建 |
| `inventory_transactions` | M2 | 出入库流水 | 未创建 |
| `document_sequences` | M3 | 单据编号序列 | 未创建 |
| `purchase_orders` | M3 | 采购订单头 | 未创建 |
| `purchase_order_lines` | M3 | 采购订单行 | 未创建 |
| `stock_in_orders` | M3 | 入库单头 | 未创建 |
| `stock_in_order_lines` | M3 | 入库单行 | 未创建 |
| `sales_orders` | M3 | 销售订单头 | 未创建 |
| `sales_order_lines` | M3 | 销售订单行 | 未创建 |
| `stock_out_orders` | M3 | 出库单头 | 未创建 |
| `stock_out_order_lines` | M3 | 出库单行 | 未创建 |
| `ap_bills` | M4 | 应付单 | 未创建 |
| `ar_bills` | M4 | 应收单 | 未创建 |
